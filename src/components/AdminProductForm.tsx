          ...
          <div className="bg-[#1b1c1d] w-max rounded-2xl text-[#ffc272] p-4 mb-20">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 mt-4">
              ...
              <div className="flex items-center gap-2 mt-6">
                <button type="submit" className="bg-[#ffc272] text-[#111213] px-3 py-1 rounded hover:bg-[#d9a556]">
                ...
                {editingProduct && (
                  <button type="button" onClick={() => {
                    ...
                    setVariants([{ taille: "", poids: 0, promotion: 0, quantity: 0 }]);
                    if (fileInputRef.current) fileInputRef.current.value = "";
                  }} className="bg-[#ffc272] text-[#111213] px-3 py-1 rounded hover:bg-red-500">
            ...
            {/* === LISTE PRODUITS === */}
            <hr className="my-20 h-1 rounded bg-[#ffffff]" />
                                ...
                                {subProducts.map((p) => (
                                  <div key={p.id} className="relative bg-[#2a2b2c] p-3 rounded-xl w-full shadow mb-2 flex gap-3">
                                    <div className="absolute top-2 right-2 flex gap-2">
                                      <button onClick={() => handleEdit(p)} className="bg-blue-600 px-2 py-0.5 rounded hover:bg-blue-500 text-white text-sm flex items-center gap-1">
                                        ✎ <span>Modifier</span>
                                      </button>
                                      <button onClick={() => handleDelete(p.id)} className="bg-red-600 px-2 py-0.5 rounded hover:bg-red-500 text-white text-sm flex items-center gap-1">
                                        🗑 <span>Supprimer</span>
                                      </button>
                                      <div
                                        onClick={() => toggleHidden(p.id, !p.is_hidden)}
                                        className={`relative inline-flex items-center cursor-pointer select-none rounded-full w-28 h-8 transition-colors duration-600 
    ${!p.is_hidden ? "bg-green-600" : "bg-gray-600"}`}
                                      >
                                        <span className={`absolute left-0 top-0 h-8 w-8 bg-white rounded-full shadow transform transition-transform duration-600
      ${!p.is_hidden ? "translate-x-20" : "translate-x-0"}`}></span>
                                        <span className="absolute inset-0 flex items-center justify-center font-medium text-sm text-white pointer-events-none">
                                          {p.is_hidden ? "Caché" : "Publié"}
                                        </span>
                                      </div>
                                    </div>
